...
def load_test_data(test_file, labels):
    ...
    test_examples = (
        df[select[0]].apply(lambda x: data_helper.clean_str(x).split(" ")).tolist()
    )
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            def real_len(batches):
                return [
                    np.ceil(np.argmin(batch + [0]) * 1.0 / params["max_pool_size"])
            ...
            def predict_step(x_batch):
                feed_dict = {
                    ...
                    cnn_rnn.pad: np.zeros(
                        [len(x_batch), 1, params["embedding_dim"], 1]
                    ),
            ...
            batches = data_helper.batch_iter(
                list(x_test), params["batch_size"], 1, shuffle=False
            )
            ...
            df.to_csv(
                predicted_dir + "predictions_all.csv",
                index=False,
                columns=columns,
                sep="|",
            )
...
if __name__ == "__main__":
    # python3 predict.py ./trained_results_1614428660/ ./data/crawler-output.csv
    predict_unseen_data()
