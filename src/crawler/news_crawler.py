...
def crawl_kompas(url, key):
    ...
    for k in pages:
        ...
        women = re.compile(
            r".*(woman|girl|girls|female|women|lady|daughter|aunt|mother|sister|wife|wives)+.*"
        )
        for i in paging:
            # page title
            title = i.find("a").text
...
if __name__ == "__main__":
    ...
    api_key = 'SECRET_KEY' # replace this with your api key
    crawl = crawl_kompas(url=web_url, key=api_key)
    print(crawl)

    # saving the dataframe
    timestamp = str(int(time.time()))
    crawl.to_csv("crawler-output.csv", sep="|", index=False)
